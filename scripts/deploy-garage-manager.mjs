...
async function main() {
  ...
  console.log(
    "Deploying GarageManager from address:",
    await wallet.getAddress()
  );

  const artifactJson = readFileSync(
    "./artifacts/contracts/GarageManager.sol/GarageManager.json",
    "utf8"
  );
  ...
  console.log("Sending GarageManager deploy transaction...");
  ...
  console.log("✅ GarageManager deployed at:", address);
}

main().catch((err) => {
  console.error("Error deploying GarageManager:", err);
  process.exitCode = 1;
});
