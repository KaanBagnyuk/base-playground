...
async function main() {
  ...
  console.log(
  "Deploying ControlStructures from address:",
  await wallet.getAddress()
);

  // Читаем артефакт контракта BasicMath, который сгенерировал Hardhat
  const artifactJson = readFileSync(
  "./artifacts/contracts/ControlStructures.sol/ControlStructures.json",
  "utf8"
);
  ...
  console.log("Sending ControlStructures deploy transaction...");
  ...
  console.log("✅ ControlStructures deployed at:", address);
}

main().catch((err) => {
  console.error("Error deploying ControlStructures:", err);
  process.exitCode = 1;
});
